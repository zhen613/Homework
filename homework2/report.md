...
## 解題策略
*    使用 Polynomial 類管理多項式，內部 Term 類儲存係數（float coef）和指數（int exp）。
*    動態陣列 termArray 儲存項，按指數降序排列，支援動態擴展（均攤 O(1)）。
*    單獨計時各操作，確保項排序，修正原始錯誤（加法指數錯誤、乘法未合併），輸出標準格式。
## 程式實作
```cpp
#include<iostream>
#include<chrono>
...
```
<img width="549" height="286" alt="螢幕擷取畫面 2025-08-01 144231" src="https://github.com/user-attachments/assets/9e08657f-3bea-4ec1-877c-d750610503d9" /><br>

## 效能分析
*    時間複雜度：O(n + m)，n 和 m 為兩多項式項數（加法）O(n * m)，合併同類項為 O((n * m) log (n * m))（乘法）。
*    空間複雜度為：O(n)，其中 n 是項數。
## 測試與驗證
| 測試案例 | 輸入多項式 $1$ | 輸入多項式 $2$ | 預期輸出（＋） | 預期輸出（＊） | 實際輸出          |
|----------|----------------|----------------|---------------|----------------|------------------|
| 測試一   | 3x³ + 2x² + x  | 2x² + x        | 3x³ + 4x² + 2x | 6x⁵ + 7x⁴ + 4x³ + x² | 3x³ + 4x² + 2x<br>6x⁵ + 7x⁴ + 4x³ + x² |
## 申論及開發報告
*    多項式表示與管理
使用 Polynomial 類別儲存多項式，內部 Term 類別表示單項（係數 coef 和指數 exp）。
動態陣列 termArray 儲存項，維護指數降序，支援動態擴展（均攤 O(1)）。
*    核心運算實現
加法：雙指針遍歷兩多項式，合併同類項（相同指數係數相加），時間複雜度 O(n + m)。
乘法：每項相乘（係數相乘，指數相加），用 map 合併同類項，時間複雜度 O(n * m)。
求值：計算每項 coef * x^exp 並求和，優化用 Horner 法降為 O(n)。
*    效能與優化
項按指數排序，加速合併。
分別計時加法、乘法、求值，確保精確效能分析。
修正原始程式錯誤（加法邏輯、乘法未合併），確保輸出如 3x² + 4x + 4（加法）、6x³ + 13x² + 8x + 3（乘法）。
